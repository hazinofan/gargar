    ...
    <section className="bg-gradient-to-b from-[#0F2345] to-[#091A36] py-12 md:py-20 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
              ...
              <div className="bg-white/90 rounded-xl md:rounded-2xl p-6 md:p-8 shadow-lg hover:shadow-xl transition-all duration-300 h-full">
                <div className="prose prose-sm sm:prose-base max-w-none">
                  {card.content.split('\n\n').map((paragraph, i) => (
                    <p 
                      key={i} 
                      className={`mb-3 md:mb-4 ${i === 0 ? 'text-lg md:text-xl font-bold text-[#0F2345]' : 'text-gray-700'}`}
                    >
                      {paragraph}
                    </p>
                  ))}
        ...
        <div className="md:hidden relative h-[500px] overflow-hidden">
          <AnimatePresence initial={false} custom={activeCard}>
            <motion.div
              key={activeCard}
              custom={activeCard}
            ...
            >
              <div className="bg-white/90 rounded-xl p-6 h-full flex flex-col shadow-lg">
                <div className="prose prose-sm max-w-none flex-grow">
                  {textCards[activeCard].content.split('\n\n').map((paragraph, i) => (
                    <p 
                      key={i} 
                      className={`mb-3 ${i === 0 ? 'text-lg font-bold text-[#0F2345]' : 'text-gray-700'}`}
                    >
                      {paragraph}
                    </p>
                  ))}
                  ...
                  <div className="flex items-center space-x-2">
                    {textCards.map((_, idx) => (
                      <button
                        ...
                        className={`w-2 h-2 rounded-full ${idx === activeCard ? 'bg-red-600 w-4' : 'bg-gray-300'}`}
        ...
        >
          <button
            className="inline-flex items-center px-6 py-2 md:px-8 md:py-3 bg-transparent border-2 border-white text-white font-bold rounded-full hover:bg-white hover:text-[#0F2345] transition-all duration-300 group text-sm md:text-base"
          >
            Share This Vision
            <svg
...
function AnimatePresence({ children, initial }: { children: React.ReactNode, initial?: boolean }) {
  return <>{children}</>;
}